...
class AttackerWithUnivector(StateMachine):
    ...
    def on_action(self, speed, robot_position, robot_vector, robot_speed,  obstacles_position, obstacles_speed,  ball_position):
        # return movement.follow_vector(255, [np.cos(robot_vector), np.sin(robot_vector)], [1, 0])
        return movement.do_univector(250, robot_position, [np.cos(robot_vector), np.sin(robot_vector)], [0, 0], np.resize(obstacles_position, (5, 2)), [[0,0]]*5, ball_position)